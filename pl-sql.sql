...
--1.pl-sql 블럭 단위 실행 >> begin, end  종결자가 있어야 하나가 실행됨
BEGIN
...
DECLARE
  v_empno emp.empno%TYPE; -- emp 테이블의 empno와 같은 타입으로  v_empno를 만들겠다
BEGIN
  select empno_seq.nextval
    into v_empno
  from dual; -- DECLARE로 v_empno 변수를 만들고 해당 변수에 시퀀스 값을 넣는다
  
  insert into empdml(empno ,ename)
  values(v_empno,'홍길동'); -- 만들어놓은 변수를 이용해 값을 넣는다
  commit;
END;
...
DECLARE
  ...
  vname varchar2(20) := null; -- 초기값 null
BEGIN -- begin~end 실행블럭
    select empno , ename , deptno
        into vempno , vename , vdeptno --변수로 값 할당하기
    from emp
    where empno=7788;
    --제어문 if(조건문){실행문}
    IF(vdeptno = 10) THEN vname := 'ACC'; -- if(vdeptno==10) { vname = "ACC"}
        ELSIF(vdeptno=20) THEN vname := 'IT'; -- elseif 아님.. elsif임.. 혼동주의!!
...
BEGIN
  LOOP
    DBMS_OUTPUT.PUT_LINE('n value : ' || n); -- 출력문
    n := n + 1; -- n += 1;
    EXIT WHEN n > 5;
  END LOOP;
END;
...
BEGIN
  FOR i IN 0..10 LOOP -- i가 0~10까지(IN : 조건절),, 자동증가
...
BEGIN
  FOR i IN 1..100 LOOP
    ...
    CONTINUE WHEN i > 5; --skip : 5 초과시 스킵
